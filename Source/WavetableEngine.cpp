                                       ...
                                       int startSample, int numSamples)
{
    ...
    for (auto& layer : oscLayers)
    {
        ...
        for (auto& v : layer.voices)
        {
            ...
            layerBuf.addFrom (0, 0, voiceBuf, 0, 0, numSamples);
            layerBuf.addFrom (1, 0, voiceBuf, 1, 0, numSamples);
        }

        float volL = layer.volume * panL;
        float volR = layer.volume * panR;
        buffer.addFrom (0, startSample, layerBuf, 0, 0, numSamples, volL);
        buffer.addFrom (1, startSample, layerBuf, 1, 0, numSamples, volR);
    }

    applyWidth (buffer, startSample, numSamples);
